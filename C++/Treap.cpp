...
class Treap
{
    ...
    public:
        ...
        /**
         * A traditional binary search function.
         *
        ...
        void bst_insert(Node * new_node)
        {
            ...
            // The treap is empty, therefore the new node is the root
            if (NULL == root)
            {
                root = new_node;
                return;
        ...
        Node * right_rotate(Node * y)
        {
            ...
            // Update the root of the treap if necessary
            if (y == root)
            {
                root = x;
            }

            // If the given node's left child has a right child, it will need to be rotated over
        ...
        Node * left_rotate(Node * x)
        {
            ...
            // If the given node's right child has a left child, it will need to be rotated over
        ...
        /**
         * Recurively fix the heap order violations of the treap.
         *
        ...
        {
            // The end of the treap has been reached (The parent should be the newly inserted node)
            if (NULL == current_node)
            {
                return NULL;
            }

            // Follow the path that the new node should be on until it is reached
            if (k < current_node->k)
            {
                // Recurively search for the newly inserted node
                current_node->left_child = treap_insert(k, current_node->left_child);

                // If the left childs priority is less than the current node's, perform a right rotation
            ...
            {
                // Recursively search for the newly inserted node
                current_node->right_child = treap_insert(k, current_node->right_child);

                // If the right childs priority is less than the current node's, perform a left rotation
        ...
        void insert(keytype k, float p)
        {
            ...
            // Don't let the priority be eqaul to "infinity"
            if (p == __FLT_MAX__)
            {
                p = p - 0.001;
            }
            ...
            // Perform a standard binary tree insert
            this->bst_insert(new_node);

            // Fix the treap if necessary
            if (num_nodes > 1)
            {
                // Recursively fixes the treap and returns the new root
        ...
        void bst_delete(keytype k)
        {
            ...
            while (NULL != current_node)
            {
                // If the key is found, it should already be a leaf therefore it can just be removed
                // from the treap
                if (k == current_node->k)
                {
                ...
                {
                    // When we go left, the nodes left subtree will be decreased by one
                    current_node->left_subtree_elms--;
                    current_node = current_node->left_child;
                }
                else
         ...
         *
         * @param[in] k            The key of the node that is going to be deleted
         * @param[in] current_node A pointer to the node to start at.
         */
        void treap_delete(keytype k, Node * current_node)
        {
            ...
            // Loop until the node to be deleted has become a leaf
            while (1)
            {

                if ((current_node->left_child == NULL) && (current_node->right_child == NULL))
                {
                    // The current node is a leaf, break out of the while loop
                    break;
                }
                else if ((current_node->left_child == NULL) && (current_node->right_child != NULL))
                {
                    // The current node only has a right child, therefore only it's priority needs to be checked

                    // If the right child priority is less, fix the treap
                    if (current_node->right_child->priority < current_node->priority)
                    {
                        current_node = this->left_rotate(current_node);

                        // If the node was not the root, then it's parents child pointers need to be updated
                ...
                {
                    // The current node only has a left child, therefore only it's priority needs to be checked

                    // If the left child's priority is less, fix the treap
                ...
                {
                    // The current node has both children, therefore all the cases need to be checked

                    // If either of the child's priorities were less, fix the treap
                    if ((current_node->left_child->priority  < current_node->priority) ||
                        (current_node->right_child->priority < current_node->priority))
                    {
                        if (current_node->left_child->priority <= current_node->right_child->priority)
                        {
                            // The left child's priority is less than or equal to the right one's,
                            // therefore a right rotation is necessary to fix the heap order violation

                            current_node = this->right_rotate(current_node);

                            // Update the parent's pointers if necessary
                            if (current_node != root)
                            {
                                if (current_node->k < current_node->parent->k)
                                {
                        ...
                        {
                            // The right child's priority is less than the left one's, therefore a
                            // left rotation is necessary to fix the heap order violation
                    ...
                    {
                        // No heap order violation was encountered, keep going down the heap
        ...
        int rank(keytype k)
        {
            ...
            while (1)
            {
                ...
                {
                    // The key was found, therefore add the left subtree elms plus one for the current node
                    return rank = rank + current_node->left_subtree_elms + 1;
                }
                else if (k < current_node->k)
                {
                    // Going left, therefore nothing needs to be added to the rank
                    current_node = current_node->left_child;
                }
                else
                {
                    // Going right, therefore add the left subtree elms plus one for the current node
        ...
        keytype select(int pos)
        {
            ...
            while (1)
            {
                ...
                {
                    // Going left, therefore the new current postion is the current node's left
                    // subtree elms plus one for the current node
                    current_node = current_node->left_child;
                    current_pos = current_node->left_subtree_elms + 1;
                }
                else
                {
                    // Going right, therefore the position to be looked for can be updated to not
                    // include nodes to the left.
        ...
        keytype successor(keytype k)
        {
            ...
            // There is no successor so return k
            if (rank_of_key == num_nodes)
            {
                return k;
            }

            // Successor's rank should be (rank_of_key + 1)
        ...
        keytype predecessor(keytype k)
        {
            ...
            // Predecessor's rank should be (rank_of_key - 1)
            return this->select(--rank_of_key);
        }
